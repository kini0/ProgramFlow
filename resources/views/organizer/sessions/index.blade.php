...
@section('content')
    <div class="flex items-center justify-between mb-2">
        ...
        <a href="{{ route('organizer.programs.sessions.create', $program) }}" class="btn-primary"><x-icon name="plus" /> Planifier une session</a>
    </div>
            ...
            <div class="card card-body flex justify-between items-center">
                <div>
                    ...
                    <p class="text-sm text-slate-500 flex items-center gap-1">
                        <x-icon name="calendar" /> {{ $s->starts_at->format('d/m/Y H:i') }} · {{ $s->facilitator?->full_name ?? 'Facilitateur ?' }}
                    </p>
                </div>
