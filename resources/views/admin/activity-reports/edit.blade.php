...
@section('content')
    <a href="{{ route('admin.programs.activityReports.show', [$program, $report]) }}" class="text-sm text-slate-500 hover:underline inline-flex items-center gap-1">
        <x-icon name="arrow-left" /> Voir le rapport
    </a>
    ...
    <form method="POST" action="{{ route('admin.programs.activityReports.update', [$program, $report]) }}" enctype="multipart/form-data" class="space-y-6 max-w-3xl">
        ...
        <button class="btn-primary"><x-icon name="floppy-disk" /> Enregistrer</button>
    </form>
    ...
    <div class="grid md:grid-cols-3 gap-4">
        @foreach($report->documents as $doc)
            <div class="card card-body">
                ...
                @else
                    <p class="text-sm mt-2 inline-flex items-center gap-1">
                        <x-icon name="paperclip" /> {{ $doc->original_name }} ({{ $doc->humanSize() }})
                    </p>
                    <a href="{{ $doc->url() }}" class="text-brand-600 text-sm hover:underline inline-flex items-center gap-1">
                        <x-icon name="download-simple" /> Télécharger
                    </a>
                @endif
